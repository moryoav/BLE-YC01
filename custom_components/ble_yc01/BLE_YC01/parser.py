...
from .const import BATT_0, BATT_100
...
class YC01BluetoothDeviceData:
    ...
    async def _get_status(self, client: BleakClient, device: YC01Device) -> YC01Device:
        ...
        if len(data) < 18:
            raise ValueError("YC01 returned an incomplete measurement frame")
        ...
        battery = round(
            100 * (self.decode_position(decodedData, 15) - BATT_0) / (BATT_100 - BATT_0)
        )
    ...
    async def update_device(self, client: BleakClient, address: str) -> YC01Device:
        """Read measurements without changing the caller-owned connection."""
        device = YC01Device(name=address, address=address)
        return await self._get_status(client, device)
